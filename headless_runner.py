...
def run_agriculture_analysis(args, weather_data):
    ...
    price_per_ton = 4800.0 if args.crop_type == 'maize' else 2500.0  # Different prices for crops
    analysis_years = 10
    discount_rate = 0.10
...
def run_coastal_analysis(args, weather_data):
    ...
    # Import coastal engine
    try:
        from coastal_engine import analyze_flood_risk
    except ImportError:
        return {
            'project_type': 'coastal',
            'error': 'Coastal engine not available',
            'message': 'coastal_engine.py module not found'
        }
...
def run_flood_analysis(args, weather_data):
    ...
    try:
        ...
        return {
            'project_type': 'flood',
            'error': 'Flood engine not available',
            'message': 'flood_engine.py module not found'
        }
